function solution =  MixFRHLP_EM(T, Y, G , K, p, ...
    q, variance_type, init_kmeans, total_EM_tries, max_iter_EM, threshold, verbose, verbose_IRLS)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% solution =  MixFRHLP_EM(T, Y, G , K, p, [options])
%
% Learn a MixFRHLP model for curve clustering and segmentation by an EM
% algorithm
%
%
% Inputs :
%
%          1. (T, Y) :  n curves each curve is composed of m points : dim(Y)=[n m]
%                * Each curve is observed during the interval [0,T]; x=[t_1,...,t_m]
...
if nargin < 6, q=1; end
if nargin < 7, variance_type = 'free';end
if nargin < 8, total_EM_tries = 10;end
if nargin < 9, init_kmeans = 1;end
if nargin < 10, max_iter_EM = 1000;end
if nargin < 11, threshold = 1e-6; end
if nargin < 12, verbose = 0; end
if nargin < 13, verbose_IRLS = 0; end
warning off
...
[n, m] = size(Y);%n  nbre de signaux (individus); m: nbre de points pour chaque signal
% % construction des matrices de regression
x = T;% linspace(0,1,m);% ou rentrer le vecteur de covariables des courbes

% pour 1 courbe
[Xbeta, Xw] = designmatrix_FRHLP(x, p, q);
...
while try_EM < total_EM_tries
    try_EM = try_EM +1;
    fprintf('EM try nbr %d\n',try_EM);
    time = cputime;
    
    % % Initialisation
    
    param = initialize_MixFRHLP_EM(curves, G , K, Xbeta, Xw, variance_type, init_kmeans, try_EM);
    ...
    while ~converge &&(iter<= max_iter_EM)
        ...
        for g=1:G
            ...
            for k = 1:K
                ...
                z=((Y-Xbeta*beta_gk).^2)/sgk;
                log_pijgk_fgk_xij(:,k) = log(pi_jgk(:,k)) - 0.5*(log(2*pi)+log(sgk)) - 0.5*z;%pdf cond ?? c_i = g et z_i = k de xij
                %pijgk_fgk_xij(:,k) = pi_jgk(:,k).*normpdf(Y,phiBeta*beta_gk,sqrt(sigma_g(k)));%---%pdf cond ?? c_i = g et z_i = k de xij
            end
        ...
        % cluster posterior probabilities p(c_i=g|Y)
        ...
        for g=1:G
            ...
            for k=1:K
                ...
                phigk = (sqrt(cluster_weights.*segment_weights)*ones(1,p+1)).*Xbeta;%[(n*m)*(p+1)]
                Ygk = sqrt(cluster_weights.*segment_weights).*Y;
                %% maximization w.r.t beta_gk: Weighted least squares
                beta_gk(:,k) = inv(phigk'*phigk + eps*eye(p+1))*phigk'*Ygk; % Maximization w.r.t betagk
                %    the same as
                %                 W_gk = diag(cluster_weights.*segment_weights);
                %                 beta_gk(:,k) = inv(phiBeta'*W_gk*phiBeta)*phiBeta'*W_gk*Y;
                %
                %% Maximization w.r.t au sigma_gk :
                if strcmp(variance_type,'common')
                    sk = sum((Ygk-phigk*beta_gk(:,k)).^2);
                    s = s+sk;
                    %ng*m = sum(sum(tauijk)) with ng = length(find(cluster_labels==g))/m;%ng = size(reshape(Yg,[],m))%
                    sigma_gk = s/sum(sum((cluster_weights*ones(1,K)).*tauijk));
                else
                    sigma_gk(k)= sum((Ygk-phigk*beta_gk(:,k)).^2)/(sum(cluster_weights.*segment_weights));
                end
            end
            ...
            %res = IRLS_MixFRHLP(cluster_weights,tauijk, phiW, Wg_init,
            %verbose_IRLS);`
            
            res = IRLS_MixFRHLP(Xw, tauijk, cluster_weights, Wg_init, verbose_IRLS);
        ...
        if prev_loglik - loglik > 1e-6
            fprintf(1, '!!!!! EM log-likelihood is decreasing from %6.4f to %6.4f!\n', prev_loglik, loglik);
            top = top+1;
            if top>20 ,  break; %solution =  MixFRHLP_EM(T, Y, G , K, p, q, variance_type, init_kmeans, total_EM_tries, max_iter_EM, threshold, verbose, verbose_IRLS)
            end
        end
        %test of convergence
...
%
for g=1:G
    solution.polynomials(:,:,g) = Xbeta(1:m,:)*solution.param.beta_g(:,:,g);
