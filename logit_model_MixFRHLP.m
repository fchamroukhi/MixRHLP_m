...
if nargin >= 3
    ...
    K = size(W,2)+1;% supposes there is no a null vector
end
if nargin>4
    Gamma = Gamma*ones(1,K);
end
...
if nargin>3    %calcul de la log-vraisemblance
    loglik = sum(sum((Gamma.*(Y.*XW)) - ((Gamma.*Y).*log(sum(expXW,2)*ones(1,K))),2));
    if isnan(loglik)
        ...
        % "log-likelihood"
        loglik = sum(sum(((Gamma.*Y).*XW) - ((Gamma.*Y).*log(sum(expXW,2)*ones(1,K)+eps)),2));
    end
    if isnan(loglik)
