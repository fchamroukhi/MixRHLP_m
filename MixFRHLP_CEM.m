function solution =  MixFRHLP_CEM(data, G , K, p, q, ... 
    variance_type, init_kmeans, total_CEM_tries, max_iter_CEM_like, threshold, verbose, verbose_IRLS) 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% solution =  curve_clustering_MixFRHLP_CEM(data,G , K, p, q, fs,[options])
%
% Learn a MixFRHLP model for curve clustering by using a Classification EM-
% like algorithm.
%
%
% Inputs : 
...
%          8. comp-loglik : at convergence of the CEM-like algo
%          9. stored_com-loglik : vector of stored valued of the
%          comp-log-lik at each CEM-like teration 
%          
...
while try_CEM_like < total_CEM_tries
    ...
    while ~converge &&(iter<= max_iter_CEM_like)
            %%%%%%%%%%%
            % E-Step  %
            %%%%%%%%%%%
...
%         log_prod_sum_pijgk_fgk_xij = zeros(n,G);
        ...
        alpha_g = param.alpha_g; % mixing prop
        for g=1:G
        ...
        [klas, c_ig] = MAP(h_ig); % c_ig the hard partition of the curves 
         ...
         comp_loglik = sum(sum(cig_log_alphag_fg_xij,2)) +  res.reg_irls ; 
        ...
        for g=1:G            
            ...
            tauijk = tau_ijgk(cluster_labels==g,:,g);%[(ng xm) x K]            
            if strcmp(variance_type,'common'),  s = 0; else sigma_gk = zeros(K,1);end
            for k=1:K    
                segment_weights = tauijk(:,k);%poids du kieme segment   pour le cluster g                  
                % poids pour avoir K segments floues du gieme cluster 
                phigk = (sqrt(segment_weights)*ones(1,p+1)).*phiBeta(cluster_labels==g,:);%[(ng*m)*(p+1)]
                Xgk = sqrt(segment_weights).*Xg;  
                ...
                %beta_gk(:,k) = inv(phigk'*phigk)*phigk'*Xgk; % Maximization w.r.t betagk
                ...
                if strcmp(variance_type,'common')
                    ...
                    sigma_gk(k)= sum((Xgk-phigk*beta_gk(:,k)).^2)/(sum(segment_weights));
                end
            end
            ...
            if ~nnz(cluster_labels==g) 
               %pause
               %warning on;
               disp('pblm of empty cluster after argmax');
               %if try_CEM_like>1
                   total_CEM_retries = total_CEM_tries-try_CEM_like+1;
               %else
...
%         subplot(212),plot(param.pi_jgk(1:m,:,2))
%         drawnow
        ...
        if verbose, 
            fprintf(1,'CEM-like   : Iteration : %d   criterion : %f \n',  iter, comp_loglik); 
        end
        if prev_comp_loglik - comp_loglik > 1e-6
            fprintf(1, '!!!!! CEM-like criterion is decreasing from %6.4f to %6.4f!\n', prev_comp_loglik, comp_loglik);
           top = top+1;   
           if top==10 break;end
        end
        %test of convergence
        converge = abs((comp_loglik-prev_comp_loglik)/prev_comp_loglik) <= threshold;
        prev_comp_loglik = comp_loglik;
        stored_comp_loglik(iter) = comp_loglik;         
    end% fin  CEM-like  loop   
    ...
    solution.stored_comp_loglik = stored_comp_loglik;
    solution.log_alphag_fg_xij = log_alphag_fg_xij;
    
    if comp_loglik > best_comp_loglik
       best_solution = solution;
       best_comp_loglik = comp_loglik;
       %solution.comp_loglik = comp_loglik;
    end 
 
        
    if total_CEM_tries>1,  fprintf(1,'max value: %f \n',solution.comp_loglik);    end
...
% BIC AIC et ICL*
loglik = sum(log(sum(exp(solution.log_alphag_fg_xij),2)));
solution.BIC = loglik - (nu*log(n)/2);%n*m/2!
solution.AIC = loglik - nu;
% ICL*
solution.ICL1 = solution.comp_loglik - nu*log(n)/2;%n*m/2!
